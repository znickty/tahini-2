...
export default function CartSidebar({ isOpen, onClose, locale }) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`
        fixed top-0 right-0 h-full w-full sm:w-96 bg-white shadow-2xl z-50 transform transition-transform duration-300
        ${isOpen ? 'translate-x-0' : 'translate-x-full'}
        ${isArabic ? 'left-0 right-auto' : 'right-0'}
      `}>
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b">
          <h2 className="text-xl font-bold text-tahini-dark flex items-center gap-2">
              ...
              {cartItems.map((item) => (
                <div key={item.id} className="flex gap-3 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="font-semibold text-tahini-dark truncate">
                      {isArabic ? item.name_ar : item.name_en}
                    </h4>
                    <p className="text-sm text-tahini-gold font-bold">
                      {item.price} SAR
                    </p>
        ...
        {cartItems.length > 0 && (
          <div className="absolute bottom-0 left-0 right-0 bg-white border-t p-4">
            ...
            <div className="space-y-2 mb-4">
              ...
              {deliveryFee > 0 && (
                <div className="text-xs text-tahini-gold">
